...
def solve_board():
    solved_board = mainSolver(level)
    for i in range(9):
        for j in range(9):
            if Board[i][j] == 0:
                addNumToBoard(solved_board[i][j], i, j, L_GREEN)
                time.sleep(0.05)
                pygame.display.flip()
    finish()
...
def draw_solve_button():
    ...
    solve_rect = solve_text.get_rect(center=(size[0] // 2, size[1] - 25))
    pygame.draw.rect(screen, GREEN, solve_rect, 5)  # Border of the button
